...
PLUGIN_DESCRIPTION = (
    "Integrates Navidrome (Subsonic) API with Picard: options page (URL, credentials, SSL), "
    "Navidrome menu/actions, playlists and library browsers with Filter, search, adjustable "
    "columns/rows and live progress, local reordering and playlist creation with a review dialog. "
    "Compatibility: Picard API 2.0–2.14; Subsonic 1.16.1+ (Navidrome); Docker Picard supported."
)
...
from picard.config import config, BoolOption, TextOption  # type: ignore
    ...
    def iter_all_songs_stream(
        ...
        *,
        ...
        while pages < max_pages:
            ...
            # Update total_known using songCount on this page
            page_total = 0
            for a in albums:
                sc = a.get("songCount", 0)
                try:
                    page_total += int(sc)
                except Exception:
                    pass
            ...
            if progress_cb:
                try:
                    progress_cb(fetched, total_known)
                except Exception:
                    pass
...
class NavidromeOptionsPage(OptionsPage):
    ...
    def _open_browser(self) -> None:
        ...
        try:
            self.save()
            client = SubsonicClient(base_url=base_url, username=username, password=password, app_name=app_name, verify_ssl=verify_ssl)
...
class NavidromeLibraryDialog(QDialog):
    ...
    def _load_songs_streaming(self) -> None:
        ...
        def _progress_cb(done: int, total_known: int) -> None:
            ...
            if total_known > 0:
                progress.set_status(f"Fetched song {done} of {total_known}")
            else:
                progress.set_status(f"Fetched song {done}…")
            if QCoreApplication is not None:
                try:
                    QCoreApplication.processEvents()
                except Exception:
...
def _clear_credentials_on_exit() -> None:
    try:
        save = False
        try:
            save = bool(config.setting.get("navidrome_save_credentials", False))
        except Exception:
